...
function vape:LoadGUI()
	...
	scarcitybanner.Text = 'Temp Hub — click the Discord icon to join our server.'
...
components = {
	...
	GUI = function(props, children, api)
		...
		discord.MouseButton1Click:Connect(function()
			local inviteUrl = (type(shared.TempHubDiscord) == 'string' and shared.TempHubDiscord ~= '' and shared.TempHubDiscord)
				or (type(vape.Discord) == 'string' and vape.Discord ~= '' and vape.Discord)
				or 'https://discord.gg/dQg8xY2xdB'
			local inviteCode = tostring(inviteUrl):match('discord%.gg/([%w%-]+)') or 'dQg8xY2xdB'
			...
			-- Prefer opening Discord / browser to OUR invite (not silent fail)
			task.spawn(function()
				local opened = false
				local tries = {
					function()
					...
					function()
						...
						if gs and gs.OpenBrowserWindow then
							gs:OpenBrowserWindow(inviteUrl)
							return true
						end
					end,
				}
				...
				-- Discord desktop RPC invite (opens Discord app on this invite)
				...
				tooltip.Text = opened and 'Opened Temp Hub Discord!' or 'Invite copied!'
				task.wait(1.2)
	...
	SettingsPane = function(props, children, api)
		...
		if props.Main then
			...
			versionlabel.Text = 'Temp Hub '..vape.Version..' '..(
