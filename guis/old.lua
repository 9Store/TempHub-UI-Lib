...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet((shared.TempHubRawBase or error('[TempHub] set shared.TempHubRawBase'))..'/'..select(1, path:gsub('newvape/', '')), true)
		end)
...
function mainapi:Load(skipgui, profile)
	...
	if isfile('newvape/profiles/'..game.GameId..'.gui.txt') then
		...
		if not guidata then
			guidata = {Categories = {}}
			self:CreateNotification('Temp Hub', 'Failed to load GUI settings.', 10, 'alert')
			savecheck = false
		end
	...
	if isfile('newvape/profiles/'..self.Profile..self.Place..'.txt') then
		...
		if not savedata then
			...
			self:CreateNotification('Temp Hub', 'Failed to load '..self.Profile..' profile.', 10, 'alert')
			savecheck = false
		end
	...
	Function = function(val, mouse)
		if mouse then
			...
			if shared.VapeDeveloper then
				...
				loadstring(game:HttpGet((shared.TempHubRawBase or error('[TempHub] set shared.TempHubRawBase'))..'/loader.lua', true))()
			end
		end
	end,
...
topbar:CreateButton({
	...
	Tooltip = 'This will set your profile to the default settings of Temp Hub'
})
topbar:CreateButton({
...
local VapeLabels = {}
local TempHubLogo = Instance.new('ImageLabel')
TempHubLogo.Name = 'Logo'
TempHubLogo.Size = UDim2.fromOffset(96, 26)
TempHubLogo.Position = UDim2.new(1, -142, 0, 3)
TempHubLogo.BackgroundTransparency = 1
TempHubLogo.BorderSizePixel = 0
TempHubLogo.Visible = true
TempHubLogo.BackgroundColor3 = Color3.new()
TempHubLogo.Image = getcustomasset('newvape/assets/old/textvape.png')
TempHubLogo.Parent = textgui.Children
...
local TempHubLogoV4 = Instance.new('ImageLabel')
TempHubLogoV4.Name = 'Logo2'
TempHubLogoV4.Size = UDim2.fromOffset(43, 30)
TempHubLogoV4.Position = UDim2.new(1, 1, 0, -2)
TempHubLogoV4.BackgroundColor3 = Color3.new()
TempHubLogoV4.BackgroundTransparency = 1
TempHubLogoV4.BorderSizePixel = 0
TempHubLogoV4.Image = getcustomasset('newvape/assets/old/textv4.png')
TempHubLogoV4.Parent = TempHubLogo
local TempHubLogoShadow = TempHubLogo:Clone()
TempHubLogoShadow.Position = UDim2.fromOffset(1, 1)
TempHubLogoShadow.ZIndex = 0
TempHubLogoShadow.Visible = true
TempHubLogoShadow.ImageColor3 = Color3.new()
TempHubLogoShadow.ImageTransparency = 0.65
TempHubLogoShadow.Parent = TempHubLogo
TempHubLogoShadow.Logo2.ZIndex = 0
TempHubLogoShadow.Logo2.ImageColor3 = Color3.new()
TempHubLogoShadow.Logo2.ImageTransparency = 0.65
local TempHubLogoGradient = Instance.new('UIGradient')
TempHubLogoGradient.Rotation = 90
TempHubLogoGradient.Parent = TempHubLogo
local TempHubLogoGradient2 = Instance.new('UIGradient')
TempHubLogoGradient2.Rotation = 90
TempHubLogoGradient2.Parent = TempHubLogoV4
...
function mainapi:UpdateTextGUI(afterload)
	...
	if textgui.Button.Enabled then
		...
		TempHubLogo.Visible = textguiwatermark.Enabled
		TempHubLogo.Position = right and UDim2.new(1 / VapeTextScale.Scale, -141, 0, 4) or UDim2.fromOffset(5, 4)
		TempHubLogoShadow.Visible = textguishadow.Enabled
		...
		VapeLabelHolder.Position = UDim2.fromOffset(4, 4 + (TempHubLogo.Visible and TempHubLogo.Size.Y.Offset or 0))
...
function mainapi:UpdateGUI(hue, sat, val, default)
	...
	if textgui.Button.Enabled then
		TempHubLogoGradient.Color = ColorSequence.new({
		...
		TempHubLogoGradient2.Color = mainapi.GUIColor.Rainbow and TempHubLogoGradient.Color or ColorSequence.new({
			ColorSequenceKeypoint.new(0, Color3.new(1, 1, 1)),
			ColorSequenceKeypoint.new(1, Color3.new(1, 1, 1))
		})
		for i, v in VapeLabels do
			v.Text.TextColor3 = customcolor or (mainapi.GUIColor.Rainbow and Color3.fromHSV(mainapi:Color((hue - ((i + 2) * 0.025)) % 1)) or TempHubLogoGradient.Color.Keypoints[2].Value)
		end
	end
